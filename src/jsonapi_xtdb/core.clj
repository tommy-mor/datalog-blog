...
;; Hello all, my name is tommy and I love clojure. You can find me on [github](https://github.com/tommy-mor) or [email me](mailto:thmorriss@gmail.com). This article was created with [clerk](https://github.com/nextjournal/clerk) and published on [clerk garden](https://clerk.garden).
;; 
...
;; The `:data` key has a vector of 10 resources, each with a type, id, some attributes, and some relationships.
...
;; At first glance this feels structure feels heavy, but it's well thought out, and handles many edge cases. I personally would prefer to expose my data using something like [pathom3](https://github.com/wilkerlucio/pathom3), but I prefer the standardized json-api approach over having arbitrary shapes that are different for every company.
...
;; Lets start an xtdb node with an empty configuration map: Instead of persisting the data in one of their [pluggable backends](https://docs.xtdb.com/administration/configuring/#_storage), we store it in plain java datstructures by giving an empty map as configuration.
...
;; Much better. We don't need to, but we could do the reverse of this operation because [section 7.8.2](https://jsonapi.org/format/#document-member-names-allowed-characters) of the json-api spec forbids "." in key names.

;; Now lets deal with toplevel responses. 

;; Because of json-api's uniformity, we can have just one function that handles every possible api response.
;; In particular, we use these properties:
...
(clerk/md "
 ...
 key `:attributes.state` with a value of `\"open\"`.
...
;; This is a nice delcarative filtering, but nothing mind shattering.
...
;; Easily rectified by calling str on the key before using it further.
